...
CGOpenMPRuntime::emitUpdateLocation(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  if ((!EmitLoc && !IsInOpenACCConstruct &&
    ...
    SrcLocStr = OMPBuilder.getOrCreateSrcLocStr(FunctionName, FileName, Line,
                                                Column, SrcLocStrSize, EndLine,
                                                FuncLine, FuncEndLine,
                                                TriggerKind);
  }
...
CGOpenMPRuntime::emitUpdateLocation(CodeGenFunction &CGF,
                                    const OMPExecutableDirective *D) {
  return emitUpdateLocation(CGF, D->getBeginLoc(), /*Flags=*/0,
                            /*EmitLoc=*/false, D);
}
...
private:
  ...
  OpenMPOffloadMappingFlags getMapTypeBits(
    ...
    if (llvm::is_contained(MapModifiers, OMPC_MAP_MODIFIER_ompx_no_alloc))
      Bits |= OpenMPOffloadMappingFlags::OMP_MAP_OMPX_NO_ALLOC;
    if (IsNonContiguous)
    ...
    if (CurTypes.end() ==
        llvm::find_if(CurTypes, [](OpenMPOffloadMappingFlags Type) {
          return !static_cast<
              ...
              Type & OpenMPOffloadMappingFlags::OMP_MAP_OMPX_NO_ALLOC);
        })) {
      CombinedInfo.Types.back() |=
          OpenMPOffloadMappingFlags::OMP_MAP_OMPX_NO_ALLOC;
    }
