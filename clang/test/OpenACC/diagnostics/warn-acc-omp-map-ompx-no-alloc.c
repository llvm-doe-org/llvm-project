...
int main() {
  int x, y, z;
  // error-error@+6  2 {{the OpenACC 'no_create' clause translation uses the 'ompx_no_alloc' map type modifier}}
  // error-note@+5   2 {{an alternative OpenMP translation can be specified with, for example, '-fopenacc-no-create-omp=no-ompx-no-alloc'}}
  // error-note@+4   2 {{or you can just disable this diagnostic with '-Wno-openacc-omp-map-ompx-no-alloc'}}
  // warn-warning@+3 2 {{the OpenACC 'no_create' clause translation uses the 'ompx_no_alloc' map type modifier}}
  // warn-note@+2    2 {{an alternative OpenMP translation can be specified with, for example, '-fopenacc-no-create-omp=no-ompx-no-alloc'}}
  // warn-note@+1    2 {{or you can just disable this diagnostic with '-Wno-openacc-omp-map-ompx-no-alloc'}}
  ...
  #pragma acc parallel
  x = 5;
  // warn-warning@+3 {{the OpenACC 'no_create' clause translation uses the 'ompx_no_alloc' map type modifier}}
  // warn-note@+2    {{an alternative OpenMP translation can be specified with, for example, '-fopenacc-no-create-omp=no-ompx-no-alloc'}}
  // warn-note@+1    {{or you can just disable this diagnostic with '-Wno-openacc-omp-map-ompx-no-alloc'}}
