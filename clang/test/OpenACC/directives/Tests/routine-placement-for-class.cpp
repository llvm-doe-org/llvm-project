...
// "acc routine seq" might be implied by implied by other member functions or
...
public:
  ...
  //     DMP: CXXMethodDecl {{.*}} decl 'void (Result *)'
  // DMP-NOT: CXXMethodDecl
  ...
  //     DMP: CXXMethodDecl {{.*}} declBeforeAfterClassUser 'void (Result *)'
  // DMP-NOT: CXXMethodDecl
  //     DMP:   ACCRoutineDeclAttr {{.*}}> Implicit Seq OMPNodeKind=unknown DirectiveDiscardedForOMP{{$}}
  // DMP-NOT:   OMPDeclareTargetDeclAttr
  //
  // PRT-NEXT: void declBeforeAfterClassUser(Result *);
  void declBeforeAfterClassUser(Result *);

  //     DMP: CXXMethodDecl {{.*}} def 'void (Result *)'
  // DMP-NOT: CXXMethodDecl
  //     DMP:   ACCRoutineDeclAttr {{.*}}> Implicit Seq OMPNodeKind=unknown DirectiveDiscardedForOMP{{$}}
  // DMP-NOT:   OMPDeclareTargetDeclAttr
  //
  ...
  //     DMP: CXXMethodDecl {{.*}} defBeforeAfterClassUser 'void (Result *)'
  // DMP-NOT: CXXMethodDecl
  //     DMP:   ACCRoutineDeclAttr {{.*}}> Implicit Seq OMPNodeKind=unknown DirectiveDiscardedForOMP{{$}}
  // DMP-NOT:   OMPDeclareTargetDeclAttr
  //
  // PRT-NEXT: void defBeforeAfterClassUser(Result *Res) {
  //      PRT: }{{$}}
  void defBeforeAfterClassUser(Result *Res) { WRITE_RESULT(Res); }
...
void Test::userAfterClass() {
  ...
  // elsewhere are sufficient to imply routine directives.
  Result Res;
  //      EXE-NEXT: declBeforeAfterClassUser: host=1, not_host=0
  //      EXE-NEXT:  defBeforeAfterClassUser: host=1, not_host=0
  //      EXE-NEXT:                   onDecl: host=1, not_host=0
  //      EXE-NEXT:                    onDef: host=1, not_host=0
  //      EXE-NEXT:                onDeclDef: host=1, not_host=0
  //      EXE-NEXT:              onDeclOnDef: host=1, not_host=0
  declBeforeAfterClassUser(&Res); PRINT_RESULT(declBeforeAfterClassUser, Res);
  defBeforeAfterClassUser(&Res); PRINT_RESULT(defBeforeAfterClassUser, Res);
  onDecl(&Res); PRINT_RESULT(onDecl, Res);
  onDef(&Res); PRINT_RESULT(onDef, Res);
  onDeclDef(&Res); PRINT_RESULT(onDeclDef, Res);
  onDeclOnDef(&Res); PRINT_RESULT(onDeclOnDef, Res);
  // EXE-HOST-NEXT: declBeforeAfterClassUser: host=1, not_host=0
  // EXE-HOST-NEXT:  defBeforeAfterClassUser: host=1, not_host=0
  // EXE-HOST-NEXT:                   onDecl: host=1, not_host=0
  // EXE-HOST-NEXT:                    onDef: host=1, not_host=0
  // EXE-HOST-NEXT:                onDeclDef: host=1, not_host=0
  // EXE-HOST-NEXT:              onDeclOnDef: host=1, not_host=0
  //  EXE-OFF-NEXT: declBeforeAfterClassUser: host=0, not_host=1
  //  EXE-OFF-NEXT:  defBeforeAfterClassUser: host=0, not_host=1
  //  EXE-OFF-NEXT:                   onDecl: host=0, not_host=1
  //  EXE-OFF-NEXT:                    onDef: host=0, not_host=1
  //  EXE-OFF-NEXT:                onDeclDef: host=0, not_host=1
  //  EXE-OFF-NEXT:              onDeclOnDef: host=0, not_host=1
...
int main(int argc, char *argv[]) {
  ...
  test.userAfterClass();
  //      EXE-NEXT:        decl: host=1, not_host=0
  ...
  //      EXE-NEXT: onDeclOnDef: host=1, not_host=0
  test.decl(&Res); PRINT_RESULT(decl, Res);
  test.def(&Res); PRINT_RESULT(def, Res);
...
#pragma acc routine seq
void Test::decl(Result *Res) { WRITE_RESULT(Res); }

// FIXME: Because Test::user*'s definitions are inline, their compute region
// uses of Test::decl*ClassUser do not imply an omp declare target for
// Test::decl*ClassUser under Clang's current OpenMP support unless Test::user*
// are used in the same compilation unit.  Either that shouldn't be required, or
// Clang's OpenACC support should diagnose it in the Clang front end.
void use_user() {
  Test test;
  test.userBefore();
  test.userAfter();
...
#pragma acc routine seq
void Test::declBeforeAfterClassUser(Result *Res) { WRITE_RESULT(Res); }
