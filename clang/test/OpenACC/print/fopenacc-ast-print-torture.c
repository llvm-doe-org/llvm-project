...
// TODO: Currently, we have no way to check a routine directive that has an
// OpenMP translation and non-empty indentation.  That's because currently a
// routine directive with an OpenMP translation is always at file scope.  In the
// future, that will change when we support routine directives within C++
