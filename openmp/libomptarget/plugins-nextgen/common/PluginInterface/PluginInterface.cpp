...
Error GenericKernelTy::launch(
  ...
  return launchImpl(GenericDevice, NumThreads, NumBlocks, KernelArgs,
                    KernelArgsPtr, AsyncInfoWrapper OMPT_SUPPORT_IF(, OmptApi));
}
...
Error GenericDeviceTy::dataSubmit(
    void *TgtPtr, const void *HstPtr, int64_t Size, __tgt_async_info *AsyncInfo
...
Error GenericDeviceTy::dataRetrieve(
    void *HstPtr, const void *TgtPtr, int64_t Size, __tgt_async_info *AsyncInfo
    OMPT_SUPPORT_IF(, const ompt_plugin_api_t *OmptApi)) {
  auto Err = Plugin::success();
  AsyncInfoWrapperTy AsyncInfoWrapper(Err, *this, AsyncInfo);
...
Error GenericDeviceTy::launchKernel(
    void *EntryPtr, void **ArgPtrs, ptrdiff_t *ArgOffsets,
  ...
  Err = GenericKernel.launch(*this, ArgPtrs, ArgOffsets, KernelArgs,
                             AsyncInfoWrapper OMPT_SUPPORT_IF(, OmptApi));
...
int32_t __tgt_rtl_get_device_type() { return Plugin::get().getOMPDeviceType(); }
...
int32_t __tgt_rtl_data_submit_async(
    ...
    OMPT_SUPPORT_IF(, const ompt_plugin_api_t *OmptApi)) {
  auto Err = Plugin::get().getDevice(DeviceId).dataSubmit(
      TgtPtr, HstPtr, Size, AsyncInfoPtr OMPT_SUPPORT_IF(, OmptApi));
  if (Err) {
...
int32_t __tgt_rtl_data_retrieve_async(
    ...
    __tgt_async_info *AsyncInfoPtr
    OMPT_SUPPORT_IF(, const ompt_plugin_api_t *OmptApi)) {
  auto Err = Plugin::get().getDevice(DeviceId).dataRetrieve(
      HstPtr, TgtPtr, Size, AsyncInfoPtr OMPT_SUPPORT_IF(, OmptApi));
  if (Err) {
...
int32_t __tgt_rtl_launch_kernel(
