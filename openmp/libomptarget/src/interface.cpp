...
static inline int targetKernel(ident_t *Loc, int64_t DeviceId, int32_t NumTeams,
  ...
  int Rc = OFFLOAD_SUCCESS;
#if OMPT_SUPPORT
  int NumClangLaunchArgs = KernelArgs->NumArgs; // might be modified by target
#endif
...
#if OMPT_SUPPORT
  // See comments in the implementation of the above target function for why
  // this ompt_target_region_exit_data callback is dispatched here.
