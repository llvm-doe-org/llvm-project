...
// RUN: %libomptarget-compile-run-and-check-host \
// RUN:   -match-full-lines -strict-whitespace --implicit-check-not=error
// RUN: %libomptarget-compile-run-and-check-x86_64-pc-linux-gnu \
// RUN:   -match-full-lines -strict-whitespace --implicit-check-not=error \
// RUN:   -check-prefixes=CHECK,X86_64
// RUN: %libomptarget-compile-run-and-check-powerpc64le-ibm-linux-gnu \
// RUN:   -match-full-lines -strict-whitespace --implicit-check-not=error \
// RUN:   -check-prefixes=CHECK,PPC64LE
// RUN: %libomptarget-compile-run-and-check-nvptx64-nvidia-cuda \
// RUN:   -match-full-lines -strict-whitespace --implicit-check-not=error \
// RUN:   -check-prefixes=CHECK,NVPTX64
// RUN: %libomptarget-compile-run-and-check-x86_64-and-nvptx64 \
// RUN:   -match-full-lines -strict-whitespace --implicit-check-not=error \
// RUN:   -check-prefixes=CHECK,X86_64,NVPTX64
// RUN: %libomptarget-compile-run-and-check-powerpc64le-and-nvptx64 \
// RUN:   -match-full-lines -strict-whitespace --implicit-check-not=error \
// RUN:   -lines -check-prefixes=CHECK,PPC64LE
...
//   CHECK-NEXT:    -1: dot=-1
//  X86_64-SAME:{{([[:space:]]+[0-9]+: dot=[0-9]+, dt=[0-9]+, tdn=[0-9]+ => x86_64)+}}
//   CHECK-NEXT:    {{[0-9]+}}: dot=-1
//   CHECK-NEXT:omp_device_ppc64le=[[#]]:
//   CHECK-NEXT:    ndot: [[#PPC64LE:]]
//   CHECK-NEXT:    -1: dot=-1
// PPC64LE-SAME:{{([[:space:]]+[0-9]+: dot=[0-9]+, dt=[0-9]+, tdn=[0-9]+ => ppc64le)+}}
//   CHECK-NEXT:    {{[0-9]+}}: dot=-1
