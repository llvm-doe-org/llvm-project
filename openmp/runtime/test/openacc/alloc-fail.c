...
// RUN:   (run-if=%run-if-x86_64  tgt-cflags=-fopenmp-targets=%run-x86_64-triple  host-or-off-pre-env=OFF  not-if-off-pre-env=%not)
// RUN:   (run-if=%run-if-ppc64le tgt-cflags=-fopenmp-targets=%run-ppc64le-triple host-or-off-pre-env=OFF  not-if-off-pre-env=%not)
// RUN:   (run-if=%run-if-nvptx64 tgt-cflags=-fopenmp-targets=%run-nvptx64-triple host-or-off-pre-env=OFF  not-if-off-pre-env=%not)
// RUN: }
// RUN: %data run-envs {
...
// RUN:         -check-prefixes=OUT,OUT-%[host-or-off-pre-env]-PRE-ENV,OUT-%[host-or-off-post-env]-POST-ENV
// RUN:   }
// RUN: }
//
// END.
...
// OUT-OFF-POST-ENV:acc_ev_enter_data_start
// OUT-OFF-POST-ENV:acc_ev_enter_data_end
// OUT-OFF-POST-ENV:acc_ev_device_shutdown_start
// OUT-OFF-POST-ENV:acc_ev_device_shutdown_end
//  OUT-OFF-PRE-ENV:acc_ev_runtime_shutdown
